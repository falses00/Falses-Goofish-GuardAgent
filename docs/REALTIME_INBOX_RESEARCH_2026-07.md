...
## 6. 2026-07-31 真实重放与长运行加固

上游 [`shaxiu/XianyuAutoAgent#75`](https://github.com/shaxiu/XianyuAutoAgent/issues/75) 记录了同一卖家消息被不同时间戳重复读取、进而产生多次请求和回复的真实故障。本项目本轮也复现出：唯一键冲突时更新 `updated_at` 会让重复买家事件重新成为最新消息，把已发送会话从 `handled` 错误投影回 `pending`。

- 买家/卖家平台事件改为 immutable insert；精确源 ID 永久幂等，并用默认 3 秒的可配置语义窗口吸收同正文但漂移时间戳的即时重放。重复包不修改正文、时间或运营排序。
- 聚合器按源消息 ID 去重，并用有序 ID 集合生成稳定批次身份。
- 原始买家事件持久关联 Outbox dedupe key；重放命中后只恢复原回复发送，不重新调用 Agent。
- 全局运营窗口按 `updated_at` 读取，使早期创建但刚刚失败的 Outbox 能浮到队首；单会话时间线仍按事件顺序读取。
- 显式搜索先在全历史中定位会话，再有界回取每个匹配会话的上下文；已验证目标事件位于 5100 条较新记录之前仍可检索。
- SSE 无变化时只读取 ChatEvent、Outbox、Takeover 三个聚合 token，变化后才生成完整快照，避免每连接每秒执行全量 Python 会话投影。

这里仍不宣称 exactly-once：组合源 ID 不是平台业务幂等键；3 秒窗口只收敛即时重放，窗口外相同正文仍被视为买家再次发言，以免永久吞掉“还在吗”等真实重复询问。
